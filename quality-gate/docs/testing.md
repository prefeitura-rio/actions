...
`pyproject.toml`, `.coveragerc`, `.coveragerc.toml`, `setup.cfg`, `tox.ini`,
and `COVERAGE_RCFILE` overrides are respected. The release script tests also verify that abbreviated commit SHAs
